"""3-seed FM ensemble candidate.

Reproducible Phase 3 bagging winner: train three official FM models with
distinct seeds and average predicted scores. Runner owns official metrics.
"""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Write 3-seed FM ensemble scores for one KuaiRand split.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    num_models = int(config.get("num_models", 3))
    ...
    for member_i in range(num_models):
        ...
        all_scores.append(np.asarray(model.predict(X_out), dtype=np.float64))
    ...
    print(
        ...
        f"seed={args.seed} num_models={num_models} k={k} lr={lr}"
    )
    return 0
